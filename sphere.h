...
vec3 Sphere::color(const ray& r, const Light& light, HitRecord& rec)const {
    ...
    float r2 = Square(material.brdf.roughness);            // 使用定义的Square
    float d2 = Square(nh2 * (r2 - 1.0f) + 1.00001f);      // 使用定义的Square
    float normalization = material.brdf.roughness * 4.0f + 2.0f;
    float spec = r2 / (d2 * ffmax(0.1f, lh2) * normalization);
    vec3 brdfval = material.brdf.specular * spec + material.brdf.diffuse;
